...
is sent and the links are written to the server log instead**, which is what you want
...
`.env` is gitignored, so keep real credentials there and never in a committed file.
...
| `DATABASE_URL` | none | Use Postgres; unset means JSON files under `data/` |
...
| `SMTP_HOST` | none | SMTP server; empty disables sending (links go to the log) |
...
**Students** can answer on their own at `/`, with no account, just the anonymous code
they already get. Nothing about that flow changed.

**Teachers** sign in at `/teacher` (email + password, with a self-hosted anti-robot
check, so no third-party captcha and no student or teacher data leaves the server).
